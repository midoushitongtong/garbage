...
const initData = async () => {
  ...
  const init = async () => {
    ...
    const qiu1GLTF = await new Promise<GLTF>((resolve) => {
      gltfLoader.load('/public/model/qiu-1.glb', resolve);
    });
    const qiu2GLTF = await new Promise<GLTF>((resolve) => {
      gltfLoader.load('/public/model/qiu-2.glb', resolve);
    });
    const qiu3GLTF = await new Promise<GLTF>((resolve) => {
      gltfLoader.load('/public/model/qiu-3.glb', resolve);
    });

    const qiu1 = qiu1GLTF.scene.children[0];
    const qiu2 = qiu2GLTF.scene.children[0];
    const qiu3 = qiu3GLTF.scene.children[0];

    qiu1.geometry.morphAttributes.position = [
      qiu2.geometry.attributes.position,
      qiu3.geometry.attributes.position,
    ];
    qiu1.updateMorphTargets();
    qiu1.morphTargetInfluences[0] = 0;
    qiu1.morphTargetInfluences[1] = 0;

    qiu1GLTF.scene.position.set(0, 1, 0);
    qiu1GLTF.scene.castShadow = true;
    ...
    const start = async () => {
      ...
      await new Promise<void>((resolve) => {
        ...
        anime({
          ...
          easing: 'linear',
          update() {
            qiu1.morphTargetInfluences[1] = targets.width;
          },
          complete() {
            resolve();
          },
